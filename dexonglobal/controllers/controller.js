...
exports.member_topup_by_admin = async (req, res, next) => {
  try {
    ...
    if (wallet_type === "fund_wallet") {
      ...
      const fund_wallet = await queryDb(
        "SELECT IFNULL(`tr03_fund_wallet`,0) AS fund_wallet FROM `tr03_user_details` WHERE `tr03_reg_id` = ? LIMIT 1;",
        [Number(getUserId?.[0]?.lgn_jnr_id)],
      );
      let q = `
...
exports.fundTransferP2P = async (req, res, next) => {
  ...
  try {
    ...
    return res
      ...
      .json(
        returnResponse(
          resMsg?.[0]?.msg === "Transactoin Successful",
          false,
          resMsg?.[0]?.msg || "Fund Transfer completed successfully.",
          [],
        ),
      );
  } catch (err) {
...
exports.ZPpayInRequest = async (req, res) => {
  ...
  const RECEIVER_WALLET =
    process.env.TRADING_POOL_COMPANY_FUND_COLLECTION_WALLET;
  ...
  const verifyPayInHash = async () => {
    if (!u_transaction_hash || !RECEIVER_WALLET) return false;
    if (!ethers.utils.isHexString(u_transaction_hash, 32)) return false;

    const provider = new ethers.providers.JsonRpcProvider(
      process.env.BSC_RPC_URL || "https://bsc-dataseed.binance.org/",
    );

    const tx = await provider.getTransaction(u_transaction_hash);
    const receipt = await provider.getTransactionReceipt(u_transaction_hash);

    if (!tx || !receipt || Number(receipt.status) !== 1) return false;

    const txContract = String(tx.to || "").toLowerCase();
    if (txContract !== String(USDT_TOKEN_ADDRESS).toLowerCase()) return false;
    ...
    if (!decoded || decoded.name !== "transfer") return false;

    const toAddress = String(decoded.args?.to || "").toLowerCase();
    const transferAmount = decoded.args?.amount;
    if (toAddress !== String(RECEIVER_WALLET).toLowerCase()) return false;
    if (!transferAmount || transferAmount.lte(0)) return false;

    const block = await provider.getBlock(receipt.blockNumber);
    if (!block?.timestamp) return false;

    const currentTimestamp = Math.floor(Date.now() / 1000);
    const ageInSeconds = currentTimestamp - Number(block.timestamp);

    if (ageInSeconds < 0 || ageInSeconds > 60) return false;
  ...
  try {
    if (Number(u_trans_status) === 2) {
      const isValidHash = await verifyPayInHash();
      if (!isValidHash) {
        return res.status(201).json({
          error: true,
          success: false,
          msg: "Invalid transaction hash, receiver, token, or timestamp.",
        });
      }
...
exports.memberPayout = async (req, res, next) => {
  ...
  try {
    ...
    if (
      (wallet_type === "growth_wallet" || wallet_type === "capital_wallet") &&
      !package_id
    ) {
      return res
        .status(201)
        .json(
          returnResponse(
            false,
            true,
            "Package id is required for growth wallet and capital wallet withdrawal.",
            [],
          ),
        );
    }
    ...
    let re =
      wallet_type === "fund_wallet"
        ? [6]
        : wallet_type === "earning_wallet" || wallet_type === "growth_wallet"
          ? [7]
          : [8];
    ...
    if (
      String(master_payout?.[0]?.m00_status || 0) == "0" ||
    ...
    if (apiRes?.data?.result?.status_text?.toLocaleLowerCase() === "complete") {
      await queryDb(
        ...
        ["XXXXXXXXXXXXXX", JSON.stringify(apiRes?.data || {}), getRandom],
      );
    } else {
      await queryDb(
        `UPDATE tr11_member_payout 
    ...
    return res
      ...
      .json(
        returnResponse(
          ...
          apiRes?.data?.message ||
            apiRes?.data?.msg ||
            "Withdrawal Successfully Proceed",
          [],
        ),
      );
    ...
    const wallet = new ethers.Wallet(
      process.env.TRADING_POOL_COMPANY_SECRET_KEY_PAYOUT,
      provider,
    );
...
exports.withdrawalApprovalFromAdmin = async (req, res) => {
  ...
  let q =
    "SELECT `tr03_inc_wallet`,`tr03_topup_wallet`,`tr03_fund_wallet` FROM tr03_user_details WHERE tr03_reg_id = ? LIMIT 1;";
  ...
  if (status_type === "Reject") {
    await queryDb(
      ...
      ["Rejected by admin", JSON.stringify({ msg: "Rejected by admin" }), t_id],
    );
    ...
    await queryDb(
      ...
      [
        ...
        "IN",
        rec?.tr11_wallet_type === "Fund Wallet"
          ? "FUND WALLET"
          : rec?.tr11_wallet_type === "Topup Wallet"
            ? "TOPUP WALLET"
            : "CASHBACK",
        open_bal,
  ...
  try {
    ...
    formData.append(
      "transactionId",
      withdrawal_record?.[0]?.tr11_transacton_id,
    );
    formData.append(
      "call_back_url",
      process.env.TRADING_POOL_DOMAIN +
      ...
      await queryDb(
        `UPDATE tr11_member_payout 
                        SET tr11_status = 1,
                            ...
                            tr11_approval_date = NOW()
                        WHERE tr11_id = ? LIMIT 1;`,
        ["XXXXXXXXXXXXXX", JSON.stringify(apiRes?.data || {}), t_id],
      );
    } else {
      await queryDb(
        `UPDATE tr11_member_payout 
    ...
    return res
      ...
      .json(
        returnResponse(
          true,
          false,
          apiRes?.data?.message || "Withdrawal Successfully Proceed",
          [],
        ),
      );
...
exports.memberDashboardBusiness = async (req, res, next) => {
  ...
  try {
    const getUserId = await queryDb("CALL sp_member_dasboard_business(?);", [
      userId || user_id,
    ]);
    return res
      .status(200)
...
exports.createUserWallet = async (req, res, next) => {
  ...
  try {
    ...
    if (!pkg_amount)
      return res
        .status(201)
        .json(returnResponse(false, true, "pkg_amount is required", []));
...
exports.perFormTransactoin = async (req, res, next) => {
  ...
  try {
    ...
    if (Number(receipt?.status) === 1) {
      ...
      if (Number(receipt?.status) === 1) {
        await queryDb(
          "INSERT INTO `tr53_transactions_records`(`tr_trans_id`,`tr_user_id`,`tr_phid`,`tr_amount`,`tr_from_wallet`,`tr_req_date`,`tr_res_date`,tr_res_params,`tr_hex_code`,`tr_status`,`tr_trans_id_out`) VALUES(?,?,?,?,?,?,?,?,?,?,?);",
          [
            transaction_id,
            Number(user_id),
            Number(user_id),
        ...
        await queryDb(q, [
          ...
          Number(
            Number(getPreBal?.[0]?.pre_bal || 0) + Number(balanceUSDT || 0),
          )?.toFixed(4),
          Number(last_id?.[0]?.last_id),
        ]);
        await queryDb(
...
exports.updateMemberProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (isBlocked !== undefined && isBlocked !== null && isBlocked !== "") {
      const status =
        isBlocked === true || isBlocked === "true" || isBlocked === "Yes"
          ? "Yes"
          : "No";
    ...
    await queryDb(
      `UPDATE tr01_login_credential SET lgn_update_prof = 'Active' WHERE lgn_email IS NOT NULL AND lgn_name <> 'N/A' AND lgn_pass IS NOT NULL AND lgn_jnr_id = ?`,
      [userId],
    );
...
exports.getNewsAndUpdated = async (req, res, next) => {
  try {
    const report = await queryDb("SELECT * FROM `m01_news_updates`;", []);
...
exports.updateNewsAndUpdated = async (req, res, next) => {
  try {
    ...
    if (id && news) {
      await queryDb(
        "UPDATE `m01_news_updates` SET `m01_nw_news` = ? WHERE `m01_nw_id` = ? LIMIT 1;",
        [news, id],
      );
    }
    return res
...
exports.updateNewsAndUpdatedStatus = async (req, res, next) => {
  try {
    ...
    if (id) {
      await queryDb(
        "UPDATE `m01_news_updates` SET `m01_nw_status` = case when `m01_nw_status` = 1 then 0 else 1 end WHERE `m01_nw_id` = ? LIMIT 1;",
        [id],
      );
    }
    return res
...
exports.createTradePair = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(
        returnResponse(true, false, "Trade pair created successfully!", []),
      );
  } catch (err) {
    console.error(err);
    next(err);
    ...
    return res
      ...
      .json(
        returnResponse(
          true,
          false,
          "Trade pair status updated successfully!",
          [],
        ),
      );
  } catch (err) {
...
exports.deleteTradePair = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(
        returnResponse(true, false, "Trade pair deleted successfully!", []),
      );
  } catch (err) {
    console.error(err);
    next(err);
...
exports.getMemberDashboard = async (req, res, next) => {
  ...
  try {
    const apiRes = await queryDb("CALL sp_member_dashboard(?);", [u_id]);

    return res
      .status(200)
      .json(
        returnResponse(true, false, "Get Member Dashboard successfully.", [
          apiRes,
        ]),
      );
  } catch (err) {
...
exports.member_global_live_transacton_activity = async (req, res, next) => {
  try {
    const getUserId = await queryDb(
      "SELECT * FROM `v_all_report` ORDER BY `tr07_created_at` DESC LIMIT 50;",
      [],
    );
    return res
      .status(200)
      ...
      await queryDb(
        `UPDATE tr11_member_payout 
                        SET tr11_status = 1,
                            ...
                            tr11_approval_date = NOW()
                        WHERE tr11_transacton_id = ? LIMIT 1;`,
        ["XXXXXXXXXXXXXX", JSON.stringify(respose || {}), respose?.trans_id],
      );
    }
...
exports.verifyTotp = async (req, res, next) => {
  try {
    ...
    if (!otp || !/^\d{6}$/.test(otp)) {
      return res
        .status(400)
        .json(returnResponse(false, true, "OTP must be a 6-digit number", []));
    }
      ...
      return res
        ...
        .json(
          returnResponse(
            false,
            true,
            "2FA is not configured on the server",
            [],
          ),
        );
    }
    ...
    if (!isValid) {
      return res
        .status(401)
        .json(returnResponse(false, true, "Invalid or expired OTP code", []));
    }

    return res
      .status(200)
      .json(returnResponse(true, false, "OTP verified successfully", []));
  } catch (err) {
    console.error(err);
    next(err);
  }
...
exports.withdrawalPermission = async (req, res, next) => {
  try {
    ...
    if (status !== 0 && status !== 1) {
      return res
        .status(400)
        .json(returnResponse(false, true, "Invalid status value", []));
    }
    ...
    if (!checkUser.length) {
      return res
        .status(404)
        .json(returnResponse(false, true, "User not found", []));
    }
    ...
    if (!id || !allowedIds.includes(Number(id))) {
      return res.status(400).json(
        returnResponse(
          false,
          true,
          "Invalid config ID. Only IDs 2–8 are allowed.",
        ), // ✅ fixed
      );
    }
    ...
    await queryDb(
      `UPDATE m00_master_config
       SET
         ...
         m00_updated_at = NOW()
       WHERE m00_id = ?;`,
      [value ?? null, status, comment ?? null, Number(id)],
    );
...
exports.updateMemberMaxPayout = async (req, res, next) => {
  try {
    ...
    if (role !== "Admin") {
      return res
        .status(403)
        .json(returnResponse(false, true, "Unauthorized access"));
    }
    ...
    return res
      .status(200)
      .json(returnResponse(true, false, "Max payout updated successfully"));
  } catch (error) {
    console.log(error);
    next(error);
  }
