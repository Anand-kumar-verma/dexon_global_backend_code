...
const {
  ...
  getPendingWalletHistory,
  getDownlineTeamTree,
  getDownlineTeamTreeAdmin,
  getDownlineTeam,
  ...
  getMemberDashboard,
  fundTransferP2P,
  memberDashboardBusiness,
  getNewsAndUpdated,
  updateNewsAndUpdated,
  updateNewsAndUpdatedStatus,
  createTradePair,
  updateTradePairStatus,
  deleteTradePair,
  getGlobalPayoutHisatory,
  member_global_live_transacton_activity,
  getTradePair,
  getPackageDetails,
  getPayoutCallback,
  updateTradeProfit,
  ...
  getMasterConfig,
  updateMasterData,
  getPayoutCallbackPayin,
  memberTopuCryptoFitpGateway,
  updateMemberMaxPayout,
...
const {
  ...
  adminReply,
  getMessaage,
} = require("../controllers/ticketAndSupport");
const {
...
router.post("/member-dapp-log-reg-with-dapp", dappUserLogin);
...
router.post(
  "/update-news-and-updates-status",
  checkAuth,
  updateNewsAndUpdatedStatus,
);
...
router.post("/update-member-max-payout", checkAuth, updateMemberMaxPayout);

// ── Admin / SubAdmin READ ─────────────────────────────────────────────────────
router.get("/get-admin-dashboard", checkAuth, isAdminSubAdmin, admin_dashboard);
router.get(
  "/get-member-global-live-transaction-activity",
  checkAuth,
  isAdminSubAdmin,
  member_global_live_transacton_activity,
);
...
router.post(
  ...
  checkAuth,
  isAdminSubAdmin,
  checkPermission("fund.withdrawal_approve"),
  withdrawalApprovalFromAdmin,
);
router.post(
  "/member-withd-perm",
  checkAuth,
  isAdminSubAdmin,
  checkPermission("fund.withdrawal_toggle"),
  withdrawalPermission,
);
...
router.post(
  ...
  checkAuth,
  isAdminSubAdmin,
  checkPermission("trade.create"),
  createTradePair,
);
router.post(
  "/update-trade-pair-status",
  checkAuth,
  isAdminSubAdmin,
  checkPermission("trade.update_status"),
  updateTradePairStatus,
);
router.post(
  "/delete-trade-pair",
...
router.post(
  ...
  checkAuth,
  isAdminSubAdmin,
  checkPermission("trade.toggle_permission"),
  tradePermission,
);
router.post(
  "/update-trade-profit",
  checkAuth,
  isAdminSubAdmin,
  checkPermission("trade.update_profit"),
  updateTradeProfit,
);
